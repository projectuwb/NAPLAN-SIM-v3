...
export const generateStatisticsQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  if (type === 'percentage') {
    ...
    return {
      ...
      workedSolution: `Percentage = (${target.count}/${total}) +ù 100 = ${percentage}%.`,
      category: 'statistics-probability',
    };
  } else {
    // Two-way table
    ...
    const distractors = makeDistractors(hiddenValue, 3);
    const options = shuffleArray([hiddenValue, ...distractors]);
...
export const generatePercentageQuantityQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  const distractors = makeDistractors(Math.floor(result), 3);
  const options = shuffleArray([Math.floor(result), ...distractors]);
...
export const generateFractionNumberQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  return {
    ...
    workedSolution: `If ${num}/${den} of the number = ${value}, then the number = ${value} +ù ${den}/${num} = ${wholeNumber}. Then ${targetNum}/${targetDen} of ${wholeNumber} = ${result}.`,
    category: 'statistics-probability',
  };
};
...
export const generateDirectionQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  const options = shuffleArray([q.result, ...distractors]);
...
export const generateEnlargementQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  if (askForWidth) {
    return {
      ...
      answerFormat: 'numeric',
      correctAnswer: newW,
      workedSolution: `Scale factor = ${newH}/${originalH} = ${scaleFactor}. New length = ${originalW} +ù ${scaleFactor} = ${newW} cm.`,
      category: 'statistics-probability',
    };
  } else {
    return {
      ...
      answerFormat: 'numeric',
      correctAnswer: newH,
      workedSolution: `Scale factor = ${newW}/${originalW} = ${scaleFactor}. New width = ${originalH} +ù ${scaleFactor} = ${newH} cm.`,
      category: 'statistics-probability',
    };
  }
};
...
export const generateProfitQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  const distractors = makeDistractors(profit, 3);
  const options = shuffleArray([profit, ...distractors]);
  ...
  return {
    ...
    mode,
    difficulty: 'intermediate',
    questionText: `Items are sold for $${sellingPrice} each. The cost to make them is: Cost = $${variableCost} +ù quantity + $${fixedCost}. How much profit is made if ${quantity} items are sold?`,
    answerFormat: 'multiple-choice',
    options: options.map((opt, i) => `${optionLabels[i]}  $${opt}`),
    correctAnswer: optionLabels[correctIndex],
    workedSolution: `Revenue = ${quantity} +ù $${sellingPrice} = $${revenue}. Cost = $${variableCost} +ù ${quantity} + $${fixedCost} = $${totalCost}. Profit = $${revenue} - $${totalCost} = $${profit}.`,
    category: 'statistics-probability',
  };
};
...
export const generatePostageQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  const options = shuffleArray([correctCost, ...distractors]);
  ...
  return {
    ...
    options: options.map((opt, i) => `${optionLabels[i]}  $${opt}`),
    correctAnswer: optionLabels[correctIndex],
    workedSolution: `Total weight = ${sheets} +ù ${weightPerSheet}g = ${totalWeight}g. This falls in the ${totalWeight <= 125 ? 'first' : totalWeight <= 250 ? 'second' : 'third'} bracket, costing $${correctCost}.`,
    category: 'statistics-probability',
  };
};
