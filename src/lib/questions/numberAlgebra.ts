...
export const generatePlaceValueQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  const distractorNumbers = makeDistractors(number, 3);
...
export const generateDecimalQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  if (type === 'division') {
    ...
    return {
      ...
      questionText: `What is the answer to ${dividend} +À ${divisor}?`,
      answerFormat: 'multiple-choice',
      options: options.map((opt, i) => `${optionLabels[i]}  ${opt}`),
      correctAnswer: optionLabels[correctIndex],
      workedSolution: `${dividend} +À ${divisor} = ${dividend * (1/divisor)} = ${correctAnswer}.`,
      category: 'number-algebra',
    };
  } else {
    ...
    return {
      ...
      mode,
      difficulty: 'intermediate',
      questionText: `Calculate ${a} +ù ${b}`,
      answerFormat: 'numeric',
      correctAnswer,
      workedSolution: `${a} +ù ${b} = ${correctAnswer}`,
      category: 'number-algebra',
    };
  }
};
...
export const generateOrderingQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  const wrongOrders = [
    ...
    shuffleArray([...decimals]).join(', '),
    shuffleArray([...decimals]).join(', '),
  ];
  
  const options = shuffleArray([correctOrder, ...wrongOrders]);
...
export const generateFractionQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  if (type === 'conversion') {
    ...
    return {
      ...
      options: options.map((opt, i) => `${optionLabels[i]}  ${opt}`),
      correctAnswer: optionLabels[correctIndex],
      workedSolution: `${whole} ${num}/${den} = (${whole} +ù ${den} + ${num})/${den} = ${improperNum}/${den}`,
      category: 'number-algebra',
    };
  } else {
    ...
    return {
      ...
      answerFormat: 'numeric',
      correctAnswer,
      workedSolution: `Halfway between ${a}/${base} and ${b}/${base} is (${a}/${base} + ${b}/${base}) +À 2 = ${a + b}/${base * 2} = ${correctAnswer}`,
      category: 'number-algebra',
    };
  }
};
...
export const generateAlgebraQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  if (type === 'simple') {
    ...
    const distractors = makeDistractors(s, 3);
    const options = shuffleArray([correctAnswer, ...distractors]);
    const correctIndex = options.indexOf(correctAnswer);
    const optionLabels = ['A', 'B', 'C', 'D'];
    ...
    const correctAnswer = `n = h +ù h - ${subtract}`;
    const distractors = [
      `n = 5 +ù h - ${subtract + 5}`,
      `n = 3 +ù h - ${subtract}`,
      `n = h +ù h + ${subtract}`,
...
export const generateEquationQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  if (type === 'missing-number') {
    ...
    return {
      ...
      mode,
      difficulty: 'basic',
      questionText: `${a} +ù h = ${b} +ù ${c}\n\nWhat is the missing number?`,
      answerFormat: 'numeric',
      correctAnswer: h,
      workedSolution: `${b} +ù ${c} = ${product}. So ${a} +ù h = ${product}, which means h = ${product} +À ${a} = ${h}.`,
      category: 'number-algebra',
    };
  } else {
    // Think of a number
    ...
    return {
      ...
      workedSolution: `Working backwards: ${result} - ${addend} = ${result - addend}. Then ${result - addend} +À ${multiplier} = ${original}.`,
      category: 'number-algebra',
    };
  }
};
...
export const generateMeanQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  const distractors = makeDistractors(addedNumber, 3);
  const options = shuffleArray([addedNumber, ...distractors]);
  ...
  return {
    ...
    options: options.map((opt, i) => `${optionLabels[i]}  ${opt}`),
    correctAnswer: optionLabels[correctIndex],
    workedSolution: `Total of ${count} numbers = ${count} +ù ${mean} = ${total}. New total = ${count + 1} +ù ${newMean} = ${newTotal}. Added number = ${newTotal} - ${total} = ${addedNumber}.`,
    category: 'number-algebra',
  };
};
...
export const generateMixedNumberQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  return {
    ...
    options: options.map((opt, i) => `${optionLabels[i]}  ${opt}`),
    correctAnswer: optionLabels[correctIndex],
    workedSolution: `Convert to improper fractions: ${improper1}/${den1} and ${improper2}/${den2}. Halfway = (${improper1}/${den1} + ${improper2}/${den2}) +À 2 = ${correctAnswer}.`,
    category: 'number-algebra',
  };
};
...
export const generatePercentageQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  const distractors = makeDistractors(percentage, 3);
  const options = shuffleArray([percentage, ...distractors]);
  ...
  return {
    ...
    options: options.map((opt, i) => `${optionLabels[i]}  ${opt}%`),
    correctAnswer: optionLabels[correctIndex],
    workedSolution: `Increase = $${newVal} - $${original} = $${increase}. Percentage increase = (${increase}/${original}) +ù 100 = ${percentage}%.`,
    category: 'number-algebra',
  };
};
