...
export const generatePerimeterQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  const distractors = makeDistractors(perimeter, 3);
  const options = shuffleArray([perimeter, ...distractors]);
  ...
  return {
    ...
    workedSolution: `For an L-shape, the perimeter equals the perimeter of the bounding rectangle: 2 +ù (${outerW} + ${outerH}) = ${perimeter} cm.`,
...
export const generateAreaQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  if (type === 'triangle') {
    ...
    return {
      ...
      workedSolution: `Area of triangle = -¢ +ù base +ù height = -¢ +ù ${base} +ù ${height} = ${area} square metres.`,
      category: 'measurement-geometry',
    };
  } else {
    ...
    return {
      ...
      workedSolution: `Side length = ÔêÜ${area} = ${side} m. Perimeter = 4 +ù ${side} = ${perimeter} m.`,
      category: 'measurement-geometry',
    };
  }
};
...
export const generateVolumeQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  if (type === 'rectangular-prism') {
    ...
    return {
      ...
      workedSolution: `Volume = length +ù width +ù height. So height = ${volume} +À (${length} +ù ${width}) = ${volume} +À ${length * width} = ${height} m.`,
      category: 'measurement-geometry',
    };
  } else {
    // Scaling question
    ...
    return {
      ...
      workedSolution: `New volume = (${originalL}/2) +ù (${originalW}/2) +ù (${originalH}+ù2) = ${originalL/2} +ù ${originalW/2} +ù ${originalH*2} = ${newVolume} cm-¦.`,
      category: 'measurement-geometry',
    };
  }
};
...
export const generateSpeedQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  if (type === 'speed') {
    ...
    return {
      ...
      options: options.map((opt, i) => `${optionLabels[i]}  ${opt}`),
      correctAnswer: optionLabels[correctIndex],
      workedSolution: `Speed = distance +À time = ${distance} +À ${time} = ${speed} m/s.`,
      category: 'measurement-geometry',
    };
  } else {
    ...
    return {
      ...
      workedSolution: `Distance = speed +ù time = ${speed} +ù (${time}/60) = ${Math.round(distance)} km.`,
      category: 'measurement-geometry',
    };
  }
};
...
export const generateScaleQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  const distractors = makeDistractors(Math.floor(actualDistance), 3);
  const options = shuffleArray([Math.floor(actualDistance), ...distractors]);
  ...
  return {
    ...
    correctAnswer: optionLabels[correctIndex],
    workedSolution: `Actual distance = map distance +ù scale = ${mapDistance} +ù ${scale} = ${actualDistance} km.`,
    category: 'measurement-geometry',
  };
};
...
export const generatePolygonQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  return {
    ...
    options: options.map((opt, i) => `${optionLabels[i]}  ${opt}`),
    correctAnswer: optionLabels[correctIndex],
    workedSolution: `Interior angle = (n-2) +ù 180-¦ / n. For ${angleSize}-¦: ${angleSize} = (n-2) +ù 180 / n. Solving gives n = ${polygon.sides}, which is a ${polygon.name}.`,
    category: 'measurement-geometry',
  };
};
...
export const generateSymmetryQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  const distractors = makeDistractors(shape.lines, 3);
  const options = shuffleArray([shape.lines, ...distractors]);
...
export const generateSurfaceAreaQuestion = (mode: 'non-calculator' | 'calculator'): Question => {
  ...
  return {
    ...
    workedSolution: `A cube has 6 faces. Each face has area ${side} +ù ${side} = ${side * side} cm-¦. Total surface area = 6 +ù ${side * side} = ${surfaceArea} cm-¦.`,
    category: 'measurement-geometry',
  };
};
